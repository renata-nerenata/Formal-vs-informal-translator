...
class Encoder(tf.keras.Model):
    ...
    def call(self, input):
        ...
        self.enc_output, self.last_hidden_state, self.last_current_state = self.lstm1(input_embedded,
                                                                                      initial_state=states)
        self.enc_output, self.last_hidden_state, self.last_current_state = self.lstm2(self.enc_output,
    ...
    def initialize_states(self, batch_size):
        ...
        self.first_hidden_state, self.first_current_state = tf.zeros([batch_size, self.lstm_size]), tf.zeros(
    ...
    def call(self, input):
        ...
        for timestep in range(loop):
            output, encoder_hidden, encoder_current = self.timestepdecoder(
                [decoder_input[:, timestep:timestep + 1], encoder_output, encoder_hidden, encoder_current])
...
class Attention_Based_Encoder_Decoder(tf.keras.Model):
    def __init__(self, input_length, inp_vocab_size, out_vocab_size, lstm_size, scoring_function, batch_size,
                 embedding_dim, embedding_matrix=None):
        super().__init__()
